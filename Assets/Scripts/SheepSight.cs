...
public class SheepSight : MonoBehaviour
{
    [Header("References")]
    ...
    void Update()
    {
        max_speed = 0.7f * sheepdog.GetComponent<DogController>().speedMovementFast;
        realSpeed = animation_speed_increment * rb.velocity.magnitude / (max_speed);
        float incSpeed = Mathf.Min(Mathf.Max(animation_speed_increment * rb.velocity.magnitude / (max_speed), 0.2f), 2.0f);
    ...
    void FixedUpdate()
    {
        ...
        // If the dog is not seen or heard follow a path with a random component for a random amount of time or stop to take a break;
        if (!dogInSight & !dogHeard)
        {
            ...
            if (!isEating)
            {
                ...
                {
                    isEating = true;
                    eatCounter = Random.Range(100, 300);
                }
            }
            if (eatCounter > 0)
            {
        ...
        if (isRunning)  // The dog has been seen
        {
            ...
            currentSpeed = Mathf.Min(sheepdog.GetComponent<DogController>().speedMovementFast * 0.6f,
